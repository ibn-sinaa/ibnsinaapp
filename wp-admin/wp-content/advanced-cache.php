...
// WP-Optimize advanced-cache.php (written by version: 3.8.0) (homeurl: https://mail.ibn-sinaa.com/) (abspath: /home/ibnsina/public_html/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/home/ibnsina/public_html/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://mail.ibn-sinaa.com/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/home/ibnsina/public_html/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-mail.ibn-sinaa.com.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-mail.ibn-sinaa.com.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-mail.ibn-sinaa.com.php');
}
