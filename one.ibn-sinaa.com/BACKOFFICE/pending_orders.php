                                ...
                                <tbody>
                                    ...
                                    try {
                                        if ($auth->hasRole(\Delight\Auth\Role::USER) || $auth->hasRole(\Delight\Auth\Role::EXTERN_USER)){
                                            $query = "SELECT order_id, order_number, branch_name, sender, order_date, delivery_date, details, price, execution_time, status, created_at FROM orders JOIN branches b on orders.branch_id=b.branch_id WHERE valid=0 AND user_id=$userId";

                                        }
                                        else if ($auth->hasRole(\Delight\Auth\Role::ADMIN)){
                                            $query = "SELECT order_id, order_number, branch_name, sender, order_date, delivery_date, details, price, execution_time, status, created_at FROM orders JOIN branches b on orders.branch_id=b.branch_id WHERE valid=0";
                                        ...
                                        if ($stmt->rowCount() > 0) {
                                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                ...
                                                if ($auth->hasRole(\Delight\Auth\Role::ADMIN)) {
                                                    echo "
                                                        ...
                                                        <button onclick='updateOrderStatus(" . ($row['order_id']) . ",-1)' class='btn btn-danger'>رفض</button>
                                                    ";
                                                }
                                                echo "
                                                    </td>";
                                            ...
                                            echo "<tr><td colspan='13'>لا توجد أي طلبات منتظرة في قاعدة البيانات</td></tr>";
                                        }
